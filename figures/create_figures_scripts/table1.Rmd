  ...
  function(data, var, weight_var, is_categorical = FALSE) {
    if (is_categorical) {
      data %>%
        ...
        select(variable, quartile, weighted_mean_exposure, median_exposure)
    } else {
      data %>%
...
```{r}
calculate_weighted_exposure_stats <- function(data, var, weight_var, is_categorical = FALSE) {
  if (is_categorical) {
    data %>%
      group_by(!!sym(var)) %>%
      summarize(
      ...
      rename(quartile = !!sym(var)) %>%
      select(variable, quartile, weighted_mean_exposure, median_exposure, weight_name)
  } else {
    data %>%
      mutate(quartile = ntile(!!sym(var), 4)) %>%
      group_by(quartile) %>%
