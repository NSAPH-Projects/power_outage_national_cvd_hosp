# Do main analysis and control for wildfire smoke
...
cvd_dlnms <- run_dlnm_models_wf_smoke(
  po_data = an_dat,
...
resp_dlnms <- run_dlnm_linear_precip_wf_smoke(
  po_data = an_dat,
  outcome_col = 'n_resp',
  exposure_cols = exposure_columns,
  offset_col = 'n_benes',
...
ggsave(
  ...
  filename = here(
    'figures_for_upload',
    'supplement',
    'wf_smoke_analysis_dlnm_jan_28.pdf'
  ),
  width = 14,
  height = 7
)
...
write_csv(tables,
          here("figures_for_upload", "supplement", "wf_smoke_results_table.csv"))